...
class Dynaformer(BaseModel2):
    def __init__(self, final_out_dim=1,hidden_dim=128, lr=1e-3, is_instance_norm=True, loss="mse", patience_lr_plateau=100):
        ...
        self.pre_encoder_conv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(32, hidden_dim, kernel_size=3, stride=1, pading=1),
        ...
        self.save_hyperparameters() # 保存所有模型、训练器、优化器、调度器超参数及日志
                                    # 存至LightningModule 的 checkpoint 中。具体来说，它会将超参数保存到 `hparams.yaml` 文件中，该文件会随着模型 checkpoint 一起保存。
                                    # 训练过程中，可以使用 `self.hparams` 来访问这些超参数。
                                    # 而学习率、优化器在预测过程中可能要重新设置
